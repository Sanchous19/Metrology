...
namespace LR3
{
    public class ProgramAnalysis
    {
        ...
        public Dictionary<string, int> OperatorsInCode { get; set; } = new Dictionary<string, int>();
        public Dictionary<string, int> Identificators { get; set; } = new Dictionary<string, int>();
        //public Dictionary<string, int> Variables { get; set; }
        public int NumberOfIdentificators { get; set; }
        public int SpenSumma { get; set; }
        public List<int> ConditionalStatementLines = new List<int>();
        public List<int> InputOutputLines = new List<int>();
        public List<string> InputOutputVariables = new List<string>();
        public List<int> InputLines = new List<int>();
        ...
        public List<string> M1 = new List<string>();
        ...
        public void CheckLines()
        {
            ...
            for (int i = 0; i < NumberOfLinesInCode; i++)
            {
                if (_code[i].StartsWith("for ") || _code[i].StartsWith("if(") || _code[i].StartsWith("if ") || _code[i].StartsWith("elif(") || _code[i].StartsWith("elif ") ||
                    _code[i].StartsWith("while(") || _code[i].StartsWith("while ") || _code[i].StartsWith("else if(") || _code[i].StartsWith("else if "))
                    ConditionalStatementLines.Add(i);
                else if (_code[i].StartsWith("print ") || _code[i].StartsWith("print("))
                    InputOutputLines.Add(i);
                if (_code[i].Contains(" input ") || _code[i].Contains(" input(") || _code[i].Contains("=input ") || _code[i].Contains("=input("))
                {
                    InputOutputLines.Add(i);
                    InputLines.Add(i);
                }
        ...
        public void SearchC()
        {
            List<string> RepeatC = new List<string>();
            for (int i = 0; i < ConditionalStatementLines.Count; i++)
            {
                string line = _code[ConditionalStatementLines[i]];
                RepeatC.AddRange(line.Split(' ').Where(str => str != ""));
            }
            foreach (var el in RepeatC)
                if (!C.Contains(el))
                    C.Add(el);
        }
        ...
        public void SearchP()
        {
            ...
            {
                string line = _code[InputLines[i]];
                int num = line.IndexOf(' ');
                if (num != -1) 
                {
                    string word = line.Substring(0, num);
                    if (!P.Contains(word))
                    {
                        P.Add(word);
                        for (int j = 0; j < NumberOfLinesInCode; j++)
                        {
                            if (!InputLines.Contains(j))
                                if (_code[j].StartsWith(word + " "))
                                {
                                    M.Add(word);
                                    P.Remove(word);
                                    break;
        ...
        public void SearchM()
        {
            List<string> RepeatM = new List<string>();
            for (int i = 0; i < NumberOfLinesInCode; i++)
            {
                string line = _code[i];
                int num = line.IndexOf(' ');
                if (num != -1)
                {
                    line = line.Remove(0, num);
                    RepeatM.AddRange(line.Split(' ').Where(str => str != ""));
                }
            }
            foreach (var el in RepeatM)
                if (!M.Contains(el))
                    M.Add(el);
            foreach (var el in C)
                if (M.Contains(el))
        ...
        public void CountInputOutputVariables()
        {
            List<string> RepeatInputOutputVariables = new List<string>();
            for (int i = 0; i < InputOutputLines.Count; i++)
            {
                string line = _code[InputOutputLines[i]];
                InputOutputVariables.AddRange(line.Split(' ').Where(str => str != ""));
            }
            foreach (var el in RepeatInputOutputVariables)
                if (!InputOutputVariables.Contains(el))
                    InputOutputVariables.Add(el);
        ...
        public void CountOperatorsInCode()
        {
            ...
            for (int i = 0; i < NumberOfLinesInCode; i++)
            {
                if (_code[i].StartsWith("def"))
                {
                    _code[i] = _code[i].Remove(0, 3);
                    _code[i] = _code[i].Insert(0, "   ");
